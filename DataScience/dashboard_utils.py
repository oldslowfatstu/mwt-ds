...
def create_stats(log_fp, dashboard_file, predictions_files=None):
    ...
    for x in (gzip.open(log_fp, 'rb') if log_fp.endswith('.gz') else open(log_fp, 'rb')):
        ...
        if x.startswith(b'{"_label_cost":'):
            ei,cost,o,ts,log_prob,a,num_a = ds_parse.json_cooked(x)
            r = 0 if cost == b'0' else -float(cost)

            # binning time stamp every 5 min
            ts_bin = get_ts_5min_bin(ts)
            ...
            d[ts_bin]['baselineRand']['d'] += 1/log_prob/num_a
            if a == 1:
                d[ts_bin]['baseline1']['N'] += 1
                d[ts_bin]['baseline1']['d'] += 1/log_prob

            if r != 0:
                d[ts_bin]['online']['n'] += r
                d[ts_bin]['baselineRand']['n'] += r/log_prob/num_a
                d[ts_bin]['baselineRand']['c'] = max(d[ts_bin]['baselineRand']['c'], r/log_prob/num_a)
                if a == 1:
                    d[ts_bin]['baseline1']['n'] += r/log_prob
                    d[ts_bin]['baseline1']['c'] = max(d[ts_bin]['baseline1']['c'], r/log_prob)

            # additional policies from predictions
            for name in pred:
                pred_prob = get_prediction_prob(a-1, pred[name][evts])     # a-1: 0-index action
                
                if pred_prob > 0:
                    p_over_p = pred_prob/log_prob
